...
export default function Table({
  ...
  const handleDeleteItems = (rows) => {
    let updatedSelectedIds = [];

    rows.map((row) => {
      updatedSelectedIds.push(row.id);
    });
  ...
  const handleSelectedRows = (id) => {
    ...
    setSelectedRows((prevSelectedRows) => {
      ...
      if (Object.keys(prevSelectedRows).length !== 0) {
        updateSelectedRows = [...prevSelectedRows, rowData];
      } else {
  ...
  const handleClickActionMenu = (id) => {
    setSelectedRows({});
    handleSelectedRows(id);
    toggleShowActionMenu();
  };

  const toggleShowActionMenu = () => {
    setShowActionMenu((prevShowActionMenu) => !prevShowActionMenu);
  };
  ...
  return (
    <>
      ...
      <div className={overflow}>
        <table className="w-full text-[11px] text-left rtl:text-right text-gray-500 min-w-full">
          <thead className="text-xs text-gray-800 uppercase bg-white sticky top-0">
            <tr>
              {hasSelectRow && (
              ...
              {Object.keys(columns).map((key) => (
                <th
                  ...
                  scope="col"
                  className="px-6 py-3 text-[13px]  text-slate-300"
                >
                  {columns[key].label}
                </th>
              ))}
                  ...
                  {hasActions && (
                    <td key={`${item.id}-actions`} className="px-6 py-4">
                      <div className="relative inline-block text-left">
                        <button
                          onClick={() => handleClickActionMenu(item.id)}
                          type="button"
                        ...
                        {showActionMenu && selectedRows[0]?.id === item.id && (
                          <div
                            id="dropdownNavbar"
                            className="z-20 w-48 font-normal absolute origin-top-right right-0 mt-2 bg-white divide-y divide-gray-100 rounded-lg shadow"
                          >
                            <ul
          ...
          {hasTotal && (
            <tfoot className="bg-white text-secondaryGray-900 font-bold text-lg sticky bottom-0">
              <tr>
